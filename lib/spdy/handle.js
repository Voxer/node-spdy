...
Handle.prototype.writev = function writev (req, chunks, allBuffers) {
  if (!this._stream) {
    var self = this
    var pendingChunks = chunks.slice()
    this.once('stream', function () {
      self.writev(req, pendingChunks, allBuffers)
    })
    return 0
  }

  return thing.prototype.writev.call(this, req, chunks, allBuffers)
}

Handle.prototype._flow = function _flow () {
  var self = this
...
Handle.prototype._closeCallback = function _closeCallback (callback) {
  ...
  if (stream._readableState.ended && !stream._writableState.finished) {
...
Handle.prototype.assignClientRequest = function assignClientRequest (req) {
  ...
  if (state.deceiver) state.deceiver.setClientRequest(req)
